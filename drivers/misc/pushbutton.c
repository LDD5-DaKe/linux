			       ...
			       size_t count, loff_t *offp)
{
	...
	if (wait_event_interruptible(data->queue,
				     (atomic_read(&data->stopflag) ||
				      !kfifo_is_empty(&data->fifo)))) {
		dev_info(data->misc.parent, "wait interrupted\n");
		return -ERESTARTSYS;
	}
	if (atomic_read(&data->stopflag)) {
		dev_info(data->misc.parent,
			 "Module is unloading, read aborted.\n");
		return -ERESTARTSYS;
	}
...
static ssize_t pushbutton_open(struct inode *inode, struct file *filep)
{
	...
	dev_info(data->misc.parent, "Waiting on Mutex to open file");
...
static ssize_t pushbutton_release(struct inode *inode, struct file *filep)
{
	struct pushbutton_dev *data =
		container_of(filep->private_data, struct pushbutton_dev, misc);
	dev_info(data->misc.parent, "Releasing File");
...
int pushbutton_probe(struct platform_device *pdev)
{
	...
	atomic_set(&pushbutton->stopflag, 0);

	regs = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (!regs) {
		dev_err(&pdev->dev, "could not get device register resource");
		return -EFAULT;
	}
...
int pushbutton_remove(struct platform_device *pdev)
{
	...
	// disable interrupts early to make sure the
	// IRQ handler does not use freed resources
	iowrite32(0, &pushbutton->button_regs->interrupt_mask);

	// set the stopbit now. No more accesses to the fifo will happen
	atomic_set(&pushbutton->stopflag, 1);
	// wake up thread currently waiting for IRQ, so it can exit
	wake_up_interruptible(&pushbutton->queue);
	dev_info(pushbutton->misc.parent, "Removing Module");

	// misc deregister will wait until the file is no longer used.
	misc_deregister(&pushbutton->misc);
	platform_set_drvdata(pdev, NULL);

	kfifo_free(&pushbutton->fifo);

	// acquire the mutex, waits until all reading threads terminated.
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(
	"Driver for the Pushbutton component Interrupt handling of the DE1-SoC Computer");
